#!/usr/bin/env bash
# Build phase for the pick_place skill: run rbnx codegen so the Driver
...
ENV_PY="${PICK_PLACE_PYTHON:-$HOME/miniconda3/envs/bb_d1_robonix/bin/python3}"
[ -x "$ENV_PY" ] && export PATH="$(dirname "$ENV_PY"):$PATH"

rbnx codegen -p "$PKG"
echo "[pick_place_skill] build done"
